...
class Master extends CI_Controller
{
  ...
  {
    $this->m_security->check();
    $data['judul'] = 'Kategori Surat';
    $data['konten'] = 'pages/kategori_surat';

   $data['id'] = $this->m_security->gen_ai_id('kategori_surat', 'id_kategori');
    $data['kategori'] = $this->m_kategori_surat->get_all();

    return $this->load->view('index', $data);
  }

  public function kategori_act($act)
  {
    $this->m_security->check();
    if ($act == 'tambah') {
      $id = $this->input->post('id');
      ...
      $query = $this->m_jenis->create($id, $nama);
      if ($query > 0) {
        $this->session->set_flashdata('pesan', '<b>Berhasil!</b> Data kategori surat telah disimpan.');
      } else {
        $this->session->set_flashdata('pesan', '<b>Gagal!</b> Data kategori surat gagal disimpan.');
      }
      redirect('/master/kategori_surat');
    } else if($act == 'ubah') {
      $id = $this->input->post('id-u');
      $nama = $this->input->post('nama-u');
      $query = $this->m_jenis->update($id, $nama);
      if ($query > 0) {
        $this->session->set_flashdata('pesan', '<b>Berhasil!</b> Data kategori surat telah diubah.');
      } else {
        $this->session->set_flashdata('pesan', '<b>Gagal!</b> Data kategori surat gagal diubah.');
      }
      redirect('/master/kategori_surat');
    } else {
      $var = explode('-', $act);
      ...
      $query = $this->m_jenis->delete($id);
      if ($query > 0) {
        $this->session->set_flashdata('pesan', '<b>Berhasil!</b> Data kategori surat telah dihapus.');
      } else {
        $this->session->set_flashdata('pesan', '<b>Gagal!</b> Data kategori surat gagal dihapus.');
      }
      redirect('/master/kategori_surat');
    }
  }
