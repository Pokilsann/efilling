...
class Laporan extends CI_Controller
{
  ...
  protected function surat_masuk($tgl1, $tgl2)
  {
    ...
    $data['subjudul'] = 'Periode: '.date("d-m-Y", strtotime($tgl1) ).' s/d '.date("d-m-Y", strtotime($tgl2));
    $data['arsip'] = $this->m_arsip_masuk->get_all();
    $this->load->view('report/pdf_template', $data);
  ...
  protected function surat_keluar($tgl1, $tgl2)
  {
    ...
    $this->load->view('report/template_surat_keluar', $data);
  ...
  protected function disposisi($tgl1, $tgl2)
  {
    ...
    $html = $this->load->view('report/template_disposisi', $data, true);
  ...
  protected function peminjaman($tgl1, $tgl2)
  {
    ...
    $html = $this->load->view('report/template_peminjaman', $data, true);
  ...
  protected function inaktif($tgl1, $tgl2)
  {
    $this->load->library('pdfgenerator');
    $data['judul'] = 'Laporan Peminjaman Arsip Surat';
    $data['subjudul'] = 'Periode: '.date("d-m-Y", strtotime($tgl1)).' s/d '.date("d-m-Y", strtotime($tgl1));
    $data['arsip'] = $this->m_inaktif->get_all();
    $html = $this->load->view('report/template_inaktif', $data, true);
  ...
  protected function retensi($tgl1, $tgl2)
  {
    ...
    $html = $this->load->view('report/template_retensi', $data, true);
